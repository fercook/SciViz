...
parser.add_argument('frameNumber', help = 'The step to process')
...
zFactor = -1.0
try :
    frame = int(args.frameNumber)
except ValueError :
    print >> stderr, 'ERROR: frameNumber is not a number. Aborting'
    stderr.flush()
    exit(-1)
# endtry
if frame < 0 :
    print >> stderr, 'ERROR: frameNumber is not a positive number. Aborting'
    stderr.flush()
    exit(-2)
# endif
try :
...
if frame < steps.__str__() != 'None' and steps.__len__() != 0 :
  scene.AnimationTime = steps[frame]
# endif
  ...
  for j in range(dimensions[1]) :\n\
    for k in range(dimensions[2]) :\n\
      ...
      if inputMaskPointData.GetValue(pointId) != "" :\n\
        pointData.SetValue(pointId, inputPointData.GetValue(pointId) )\n\
      else :\n\
...
outputFileName = splitext(basename(args.inputFile) )[0] + '_' + args.fieldName +  '_' + str(frame).zfill(5)
if dataInfo != None :
  ...
  print 'OK: [FR:' + str(frame) + ', FD:' + args.fieldName + ', DIM:' + args.dimensions + ']: ' + outputFileName
  stdout.flush()
  exit(0)
# endif
print >> stderr, 'KO: [FR:' + str(frame) + ', FD:' + args.fieldName + ', DIM:' + args.dimensions + ']: ' + outputFileName
stderr.flush()
exit(1)
