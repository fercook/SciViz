...
def colorMesh(obj,mesh,colorFile):
   ...
   if (len(valueList[1]) != len(mesh.vertex_colors[0].data) or minVal == maxVal):
       print ("Color file does not correspond to num of vertices")
       print ("Read" + str(len(valueList[1])) + ", expected "+ str(len(mesh.vertex_colors[0].data)))
       return
   ...
   for i in range(len(mesh.vertex_colors[0].data)):
       color1 = ramp.evaluate((valueList[1][i]-minVal)/(maxVal-minVal))[0:3]
       mesh.vertex_colors[0].data[i].color = color1
